...
def _write_png(layer: Layer, path: Path, width: int, height: int) -> None:
    rgb = layer.rgb
    alpha = layer.alpha
    if layer.bbox is not None:
        ...
        a[y0:y1, x0:x1] = alpha if alpha is not None else 1.0
        alpha = a
    if alpha is None:
...
def write_pngjsx(stack: LayerStack, out_dir: Path) -> Path:
    ...
    def emit(layer: Layer, group: str | None):
        ...
        fname = f"{safe}.png"
        _write_png(layer, layers_dir / fname, stack.width, stack.height)
...
def _generate_jsx(stack: LayerStack, manifest, group_vis) -> str:
    lines = [
        ...
        "function placeLayer(relPath, name, blend, visible, targetSet) {",

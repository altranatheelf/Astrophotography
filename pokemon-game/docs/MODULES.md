...
Four, none of which the engine knows anything about. Each one is `js/modules/<id>/`
...
| **`bullet`**<br>*A fight, as an add-on* | A bullet-hell scene written against the engine's public API only, to find out what the engine owed a module that is not a story: patterns, movers, spawn/steer/step, one scene. | `save.modules.bullet` · `project.packs.bullet` | scene `bullet-fight` · `KIT.bullet` | `js/modules/bullet/README.md` — what it cost |

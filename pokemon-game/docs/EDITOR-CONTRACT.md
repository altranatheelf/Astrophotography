...
| `KIT.editor.inspector.packForm(host, prev, { project, fields, value, at, label })` | A module's own settings as a schema form. Each change writes the one field that changed under `at` as one undo step. While focus is in the form it is only re-read, so the caret survives the refresh its own commit causes; otherwise it is rebuilt, so a picker lists what was imported since. The Dungeon form and every panel `tools/new-module.js` wrote used to pass the field's path to `doc.set` as the whole pack. |

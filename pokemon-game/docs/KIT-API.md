...
`KIT.look.resolve(project) -> look` — every field filled in: `tokens` (colours, widths, fonts), `sounds` (one per role), `voice`, `base`, plus `_chain` (`['kit', …]`, root first) and `_problems`. The layers are the schema defaults, the `kit` look, the chain of looks it starts from (`project.looks` first, then the `looks` registry; a missing base, a loop or a chain more than eight deep stops the walk there and says so), then `project.ui`. Every layer is sanitised on the way in.
`compile(look, env) -> css` — pure and deterministic; only what differs from the kit, as custom properties on `#stage`, so **the kit look compiles to `''`** and a game that never touched its look ships exactly `css/kit.css`; `env` is `{ fonts }`, the game's `project.fonts` · `css(project) -> css` — `compile(resolve(project), { fonts: project.fonts })`, what `apply` writes into the page · `sanitize(ui) -> { ui, dropped }` (hex colours, numbers pulled into range, known enums, ids; anything else is dropped, and there is no field for raw CSS) · `problems(project)` (what the `ui` validator reports: `ui-base-missing`, `ui-base-cycle`, `ui-base-deep`, `ui-bad-value`, each a warning with `where: { look: true, path }`).
...
`TOKENS` (schema fields, each with the `css` property it sets and the `fallback` literal `css/kit.css` writes after the comma; `test/kit/look.test.js` holds the two to each other) · `PARTS` (`sounds`) · `ROLES` (`move confirm cancel buzzer save open close page toast` and the kit's sound for each; null is silence) · `events` (`changed` { look }, `applied` { css, look }) · the `looks` registry (`{ id, label, describe, rev, base, ui }`; `js/kit/ui/presets.js` registers `kit`).

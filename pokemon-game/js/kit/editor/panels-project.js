...
//   Rules      the rules of the world: when, what, and whether one is switched off
...
//   Import     drop a Tiled map, an RPG Maker MV/MZ folder, Aseprite art, a PNG, a sound,
//              or anything a registered importer reads, in here
//
...
(function (root) {
  ...
  PP.usageLabel = function (project, where) {
    ...
    if (w.map) {
      ...
      if (w.slot) bits.push(P.slotLabel(w.slot));
      return bits.join(' · ');
    }
  ...
  /**
   * textIntake(body, { accept, placeholder, button, title }, onText, say) — a
   * file picker and a paste box, the two ways a phone can hand over a text
   * file. onText(text, name) gets the file's name, or no name for a paste.
   */
  function textIntake(body, o, onText, say) {
    const inRow = make('div.ed-row');
    const file = make('input');
    file.type = 'file';
    file.accept = o.accept;
  ...
  // ---- the Project panel's sections, one function each ------------------------

  function gameSection(body, p, forms) {
    const box = make('div');
  ...
  function startPointSection(body, p, forms) {
    const box = make('div');
    body.appendChild(box);
    forms.push(INS().mount(box, {
  ...
  function settingsSection(body, p, forms) {
    const box = make('div');
    body.appendChild(box);
    forms.push(INS().mount(box, {
  ...
  function otherDeviceSection(body, p) {
    ...
    textIntake(body, { accept: '.json,.kitgame.json,application/json', placeholder: 'or paste a game here', button: 'Open what is pasted', title: 'Replace this game with the pasted one' },
      (text, name) => openGame(text, name || 'the pasted game', say), say);
  ...
  function newGameSection(body, p) {
  ...
  // it back.
  function languagesSection(body, p) {
    const L = KIT.lang;
    ...
    if (code && p.languages && p.languages[code]) {
      ...
      textIntake(body, { accept: '.txt,text/plain', placeholder: 'or paste a translated file here', button: 'Take what is pasted', title: 'Read the translations out of it' },
        (text, name) => take(text, name || 'What you pasted'), say);
  ...
  function modulesSection(body, p) {
  ...
  KIT.registry('editorPanels').add({
    ...
    refresh(ed) {
      ...
      section(host, 'The game', true, (body) => gameSection(body, p, forms));
      section(host, 'Heroes', true, (body) => heroesSection(body, p));
      section(host, 'Where the game starts', true, (body) => startPointSection(body, p, forms));
      section(host, 'Settings', false, (body) => settingsSection(body, p, forms));
      section(host, 'This game, on your other device', moveState.open === true, (body) => otherDeviceSection(body, p), moveState);
      section(host, 'Start a new game', newState.open === true, (body) => newGameSection(body, p), newState);
      section(host, 'Languages', langState.open === true, (body) => languagesSection(body, p), langState);
      section(host, 'Modules', false, (body) => modulesSection(body, p));
    },
  });
  ...
  KIT.registry('editorPanels').add({
    ...
    refresh(ed) {
      ...
      if (this._form) { this._form.destroy(); this._form = null; }
      clear(el.detail);
  ...
  KIT.registry('editorPanels').add({
    ...
    mount(host) {
      clear(host);
      const el = this._el = {};
      // Picked or dropped, the files go the same way, and a failure says so the same way.
      const takeFiles = (files) => Promise.resolve(read(this, files)).catch((err) => {
        clear(el.report);
      ...
      el.file.onchange = () => { if (el.file.files && el.file.files.length) takeFiles(Array.from(el.file.files)); };
  ...
  function renderReport(panel, found, report, result) {
    ...
    const apply = btn(errs.length ? 'Import anyway' : '✓ Import it', 'Merge this into the project as one undo step', () => {
      ...
      ED.afterEdit();                                         // merge wrote the document itself: refresh, validate, save
